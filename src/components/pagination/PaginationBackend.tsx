...
interface Props {
  ...
  currentPageFunc: (arg: number) => void;
}

const PaginationBackend: React.FC<Props> = ({
  currentPage,
  currentPageFunc,
  dataLength,
  postsPerPage = 5,
  ...
  const nextPagination = () => {
    if (currentPage === highestNumber) {
      return currentPageFunc(currentPage);
    }
    currentPageFunc(currentPage + 1);
